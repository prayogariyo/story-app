...
class EditTextPassword : AppCompatEditText, View.OnTouchListener {
    ...
    private fun init() {
        ...
        passwordButtonImage = ContextCompat.getDrawable(context, if (!isPasswordVisible) R.drawable.baseline_remove_red_eye_24 else R.drawable.baseline_lock_24 ) as Drawable
            ...
            if(isPasswordButtonClicked){
                when(event.action) {
                    MotionEvent.ACTION_DOWN -> {
                        passwordButtonImage = ContextCompat.getDrawable(context, if(isPasswordVisible) R.drawable.baseline_lock_24 else R.drawable.baseline_remove_red_eye_24) as Drawable
                        setEditCompoundDrawables(endOfTheText = passwordButtonImage, startOfTheText = passwordIcon)
